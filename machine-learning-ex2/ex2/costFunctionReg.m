...
%J = 1/m*sum(-y.*log(sigmoid(X(:,1)*theta(1,1)))-(1-y).*log(1-sigmoid(X(:,1)*theta(1,1))));
...
for i = 2:size(theta,1)
    grad(i,1) = 1/m*sum(X(:,i)'*(sigmoid(X*theta)-y))+(lambda/m)*theta(i,1);
end
%grad(3,1) = 1/m*sum((sigmoid(X*theta)-y).*X(:,3))+(lambda/m)*theta(2,1);
